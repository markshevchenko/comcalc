...
step2_generator::step2_generator(const table_registry& table_registry, std::ostream& out) : _out(out) {
	...
	_input_only_variables = set_except(input_variables, output_variables);
	_output_only_variables = set_except(output_variables, input_variables);
	_all_variables = set_union(input_variables, output_variables);
...
std::set<std::string> set_except(const std::set<std::string>& from, const std::set<std::string>& what) {
	auto result = from;
...
std::set<std::string> set_union(const std::set<std::string>& what, const std::set<std::string>& with) {
	auto result = what;
...
void step2_generator::visit_unary_operator(const ast_unary_operator* unary_operator) {
	...
	int index = get_next_variable_index();
	if (operand._type == expression_type::DOUBLE)
